...
def generate_exit_sets(node: Node, entry_set: apron.Abstract0, exit_sets: Dict[str, apron.Abstract0],
    ...
    if node_module is None:
        ...
        # BUT skip reinjection for loop back-edge nodes
        # (ForLoopContinue, ForLoopJoin) because widening has
        # intentionally produced wider intervals for variables
        # modified inside the loop body (e.g. result = result * 2).
        # Reinjecting the initial registry value here would destroy
        # the widened interval and prevent overflow detection.
        is_loop_node = (node.node_type in ('ForLoopContinue', 'ForLoopJoin'))
        if not is_loop_node:
