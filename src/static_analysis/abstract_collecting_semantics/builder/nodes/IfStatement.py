...
from static_analysis.abstract_collecting_semantics.builder.common import compute_expression_object, generate_bottom_state, Bottom
...
def generate_exit_sets(node: IfStatement, entry_set: apron.Abstract0, exit_sets: Dict[str, apron.Abstract0],
    ...
    if expr_value == 'any' or isinstance(expr_value, Bottom):
        # 'any'   → condition indeterminate at analysis time; sound over-approx.
        # Bottom  → a variable in the condition is unregistered (e.g. synthetic
        #           reentrancy variables BAL/attacker_bal whose probe constants
        #           are set in the constant_registry but whose Apron Texpr0 node
        #           cannot be resolved at this point in the fixpoint iteration).
        #           Propagate the entry state to BOTH branches (sound widening).
    ...
    elif expr_value is False:
        exit_dict[false_branch] = entry_set
        # exit_dict[true_branch] = generate_bottom_state(var_registry, manager)
    else:
        # Any other non-boolean result (e.g. a Texpr0Intern object returned when
        # the condition is a numeric expression rather than a comparison) is also
        # treated as indeterminate: propagate to both branches.
        print(f"[IfStatement] Condition indeterminate ({type(expr_value).__name__}), "
              f"propagating to both branches.")
        exit_dict[true_branch] = entry_set
        exit_dict[false_branch] = entry_set
