...
def generate_exit_sets(node: ForStatement, entry_set: apron.Abstract0, exit_sets: Dict[str, apron.Abstract0],
    ...
    # If entry state is bottom, propagate bottom to both branches
    if entry_set.isBottom(manager):
        return {true_branch: entry_set, false_branch: entry_set}
    ...
    if isinstance(expr_value, Bottom):
        # Indeterminate: send to BOTH branches
        print(f"[ForStatement] Condition indeterminate (Bottom), sending state to both branches")
        ...
        # Also treat non-boolean results as indeterminate
        print(f"[ForStatement] Condition non-boolean ({expr_value}), sending state to both branches")
        exit_dict[true_branch] = entry_set
        exit_dict[false_branch] = entry_set
