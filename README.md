...
Our tool is a static analyzer for Solidity smart contracts that detects
**reentrancy**, **integer overflow/underflow**, **timestamp dependence**, and
**transaction-ordering dependence (TOD)** by *abstract interpretation* over
several numerical domains — **Interval (Box)**, **Octagon**, and **Polyhedra
(Polka)**. On top of the abstract fixpoint it adds a **semantics-based
feasibility-confirmation layer** that discards a warning when the abstract state
proves the offending path infeasible, which reduces false positives. The
underlying abstract semantics are a *sound over-approximation* of all reachable
states; as the paper notes, this soundness is a property of the
over-approximation rather than a guarantee of complete detection.
...
1. **Semantic preprocessing for solidity-specific constructs.** Smart-contract
   features that have no direct numerical meaning — mappings, structs, balance
   transfers, and low-level external calls — are rewritten into an equivalent
   *scalar* form that the numerical domains can reason about, while keeping the
...
2. **Multi-domain feasibility confirmation.** A vulnerability is first detected
   structurally/semantically, then *confirmed* by checking whether the abstract
...
| Pipeline                       | Source analyzed                                                          | Domain(s)                                   | Verdict criterion                                                              |
| ------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------------------ |
| **Reentrancy**           | semantically transformed (balance/credit injected, call-back edge added) | Box, Octagon, Polka                         | Δ-width mismatch on`{BAL, attacker_bal, credit}` across the re-entrant edge |
| **Overflow / underflow** | original                                                                 | Box                                         | computed interval`[lo, hi]` vs the declared type range                       |
| **Timestamp dependence** | transformed                                                              | Polyhedra (Polka)                           | control/data influence of`block.timestamp` on a sensitive sink               |
| **TOD**                  | transformed                                                              | Polyhedra (Polka)                          | order-dependent state/transfer flagged by the dependency engine                |
